...
router.get("/chitiet/:id", (req, res) => {
  ...
  const sql = `
    SELECT * FROM table_chitietnguoidung
    WHERE nguoidung_id = ?
  `;

  db.query(sql, [nguoidung_id], (err, result) => {
    if (err) {
      console.error("❌ Lỗi khi lấy chi tiết người dùng:", err);
      return res
        .status(500)
        .json({ error: "Lỗi khi lấy chi tiết người dùng!" });
    }
    if (result.length === 0) {
      return res.status(404).json({ error: "Không tìm thấy người dùng!" });
    }
    res.status(200).json(result[0]); // chỉ trả về 1 đối tượng
  });
});
